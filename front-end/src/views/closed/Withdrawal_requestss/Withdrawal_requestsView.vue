...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
    <!-- Loading -->
    <Loading :visible="loading" message="Loading Withdrawal_requests..." />

    <!-- Page Header -->
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">Withdrawal Requests</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Amount</th><th class="px-6 py-3 text-left">User</th><th class="px-6 py-3 text-left">Status</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-green-50 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.amount }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.user.id }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.status }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">Withdrawal_requests #{{ index + 1 }}</h2>
    ...
    <!-- Add/Edit Modal -->
    <add-withdrawal_requests v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <edit-withdrawal_requests v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>

    <!-- Delete Confirmation Modal -->
    <delete-confirm-modal 
      :visible="deleteModalVisible"
      title="Delete Withdrawal_requests"
      message="Are you sure you want to delete this Withdrawal_requests?"
...
<script>
import AddWithdrawal_requests from "./AddWithdrawal_requests.vue";
import EditWithdrawal_requests from "./EditWithdrawal_requests.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddWithdrawal_requests, EditWithdrawal_requests, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/get_withdrawal_requests', params);
        console.log("response with drawal requests",response)
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'Withdrawal_requests-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/withdrawal_requests', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('Withdrawal_requests deleted successfully', 'success');
      }
