...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
    <!-- Loading -->
    <Loading :visible="loading" message="Loading Event..." />
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Description</th><th class="px-6 py-3 text-left">Start_date</th><th class="px-6 py-3 text-left">End_date</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-green-50 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.description}}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.start_date }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.end_date }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">Event #{{ index + 1 }}</h2>
    ...
    <!-- Add/Edit Modal -->
    <add-event v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <edit-event v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
...
export default {
  components: { AddEvent, EditEvent, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/get_events', params);
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/delete_event', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('Event deleted successfully', 'success');
      }
