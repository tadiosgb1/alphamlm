  ...
  <div class="p-4 bg-gray-50 min-h-screen text-sm text-gray-800">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3">
      ...
      <div class="flex items-center gap-2 text-sm text-gray-600">
        <label>Show</label>
        <select v-model="pageSize" @change="fetchCategories(1)" 
        ...
        <table class="min-w-full text-sm hidden sm:table">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap text-gray-500">{{ formatDate(category.created_at) }}</td>
              <td class="px-6 py-4 whitespace-nowrap space-x-3">
                <button class="text-green-500 hover:text-green-700 transition duration-150" @click="openEdit(category)" title="Edit">
                ...
                <button class="text-red-500 hover:text-red-700 transition duration-150" @click="deleteCategory(category.id)" title="Delete">
            ...
            <tr v-if="categories.length === 0">
              <td colspan="5" class="text-center py-6 text-gray-400 italic">No categories found.</td>
            </tr>
          </tbody>
        </table>
      </div>
        ...
        >
          <div class="flex justify-between items-start">
            ...
            <div class="flex gap-3 text-sm">
              <button class="text-green-500 hover:text-green-700" @click="openEdit(category)">
              ...
              <button class="text-red-500 hover:text-red-700" @click="deleteCategory(category.id)">
          ...
          <p class="text-gray-600 text-xs mt-1">{{ category.description }}</p>
          <p class="text-gray-500 text-xs mt-2 pt-2 border-t border-gray-100">
            <span class="font-medium">Created:</span> {{ formatDate(category.created_at) }}
          </p>
          <p class="text-xs text-gray-500 mt-1">#{{ (currentPage - 1) * pageSize + index + 1 }}</p>
        </div>
        <div v-if="categories.length === 0" class="text-center py-4 text-gray-400 italic">No categories found.</div>
      </div>
    </div>
...
        
        // Assuming API returns results directly or in a paginated object
        this.categories = res.results || res.data || [];
        this.count = res.count || this.categories.length;
...
    formatDate(dateStr) {
      if (!dateStr) return '-';
      // Added a more robust date formatting check
      try {
        ...
        return dateStr; // Return raw string if formatting fails
      }
    }
  },
  mounted() {
...
<style scoped>
/* Ensuring text doesn't wrap in the table */
