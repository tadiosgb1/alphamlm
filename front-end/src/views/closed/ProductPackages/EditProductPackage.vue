<template>
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 text-sm">
      <h2 class="font-bold text-lg text-gray-800 mb-4 border-b pb-2">Edit Product Package</h2>
...
    productPackage:{ 
        immediate:true, 
        handler(val){ 
            // Initialize form data when productPackage prop changes
            if(val) this.form={ 
                id: val.id, 
                product: val.product, 
                package: val.package 
...
    async updatePP(){ 
        ...
        try{ 
            const res= await this.$apiPut(`/update_product_package`, this.form.id, this.form); 
           if(res){
    ...
            this.$emit('saved'); 
            ...
            console.error(err); 
            this.$root.$refs.toast.showToast("Product Package edit failed", "error");
          } 
    }
  }
}
