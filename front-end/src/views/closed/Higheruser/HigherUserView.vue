...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
    ...
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">HigherUser</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Phone_number</th>
             
              <th class="px-6 py-3 text-left">
                First_name</th>
                <th class="px-6 py-3 text-left">Middle_name</th>
                <th class="px-6 py-3 text-left">Last_name</th>
                ...
                <th class="px-6 py-3 text-left">Bank name</th>
                 ...
                 <th class="px-6 py-3 text-left">Tree Position</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-green-50 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.email }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.phone_number }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.password }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.first_name }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.middle_name }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.last_name }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">HigherUser #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-700">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Tree Position:</span>
              {{ item.last_name }}
            </div>
        </div>
      </div>
    ...
    <!-- Add/Edit Modal -->
    <AddHigherUser v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <EditHigherUser v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>

    <!-- Delete Confirmation Modal -->
    <delete-confirm-modal 
      :visible="deleteModalVisible"
      title="Delete HigherUser"
      message="Are you sure you want to delete this HigherUser?"
...
<script>
import AddHigherUser from "./AddHigherUser.vue";
import EditHigherUser from "./EditHigherUser.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddHigherUser, EditHigherUser, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/higheruser', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'HigherUser-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/higheruser', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('HigherUser deleted successfully', 'success');
      }
